...
export function comboStats(rows: Attack[], minAttacks = 1): ComboRow[] {
  ...
  return Array.from(map.entries()).map(([key, items]) => {
    ...
    const defensive_score = +((100 - hr) * Math.log10(attacks || 1)).toFixed(2)
    return { combo: `${title(base_style)} + ${title(spell_tower)}`, base_style, spell_tower, attacks, triples, hr, avg_stars, avg_percent, defensive_score }
